...
Deno.serve(async (req) => {
  ...
  try {
    const { email, password, capability }: ReqPayload = await req.json();
    ...
    if (!userId) {
      return json(
        { error: "Reset verification is invalid or already used." },
        403,
      );
    }
    ...
    if (consumeError) {
      console.error("password reset capability consumption failed", consumeError);
      return json({ error: "Could not validate reset verification." }, 500);
    }
    if (consumeStatus === "expired") {
      return json({ error: "Reset verification expired. Request a new code." }, 410);
    }
    if (consumeStatus !== "ok") {
      return json({ error: "Reset verification is invalid or already used." }, 403);
    }
