-- Minimal production-safe repair for released clients that already call the
-- audience-aware v3 post and event RPCs. Keep this migration additive and
-- narrowly scoped: unrelated follower-role and chat hardening stays in its
-- original rollout.
...
begin
  if not exists (
    ...
    where conrelid = 'public.club_posts'::regclass
      and conname = 'club_posts_audience_check'
  ) then
    alter table public.club_posts
      add constraint club_posts_audience_check
      check (audience in ('everyone','followers','board')) not valid;
    alter table public.club_posts
      validate constraint club_posts_audience_check;
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.events'::regclass
      and conname = 'events_audience_check'
  ) then
    alter table public.events
      add constraint events_audience_check
      check (audience in ('everyone','followers','board')) not valid;
    alter table public.events
      validate constraint events_audience_check;
  end if;
end
$$;
...
grant execute on function private.can_view_club_content(uuid,text)
...
begin
  ...
  select coalesce(nullif(name,''),'A club') into v_club_name
    from public.clubs where id=p_club_id;
  perform private.enqueue_notification_outbox_v2(
    'club_post:'||v_post.id,'club_post',v_actor,'post',v_post.id,
    case p_audience when 'board' then 'club_board' else 'club_followers' end,
    p_club_id,
    jsonb_build_object('exclude_recipient_ids',to_jsonb(coalesce(p_mentioned_user_ids,'{}'::uuid[]))),
    v_club_name||' posted something new',
...
begin
  ...
  insert into public.events(
    id,club_id,title,description,location,event_date,starts_at,ends_at,
    is_public,created_by_user_id,image_path,image_url,tags,registration_url,
    schedule,speakers,audience
  ) values(
    p_event_id,p_club_id,btrim(p_title),p_description,p_location,p_event_date,
    p_starts_at,p_ends_at,true,v_actor,p_image_path,p_image_url,
    coalesce(p_tags,'{}'),p_registration_url,p_schedule,
    coalesce(p_speakers,'[]'),p_audience
  ) returning * into v_event;
  select coalesce(nullif(name,''),'A club') into v_club_name
    from public.clubs where id=p_club_id;
  perform private.enqueue_notification_outbox_v2(
    'club_event:'||v_event.id,'club_event',v_actor,'event',v_event.id,
    case p_audience when 'board' then 'club_board' else 'club_followers' end,
    p_club_id,'{}','New event from '||v_club_name,
...
create or replace function public.set_club_post_audience_v2(
  p_post_id uuid,
  p_audience text
...
create or replace function public.set_club_event_audience_v2(
  p_event_id uuid,
  p_audience text
) returns boolean language plpgsql security invoker set search_path = '' as $$
declare v_club uuid;
